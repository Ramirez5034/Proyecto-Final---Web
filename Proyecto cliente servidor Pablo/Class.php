...
$consulta = "SELECT C.ClassID, E.FirstName AS EmployeeName, Co.Name AS CourseName, P.Name AS PeriodName
             FROM Class C
             INNER JOIN Employee E ON C.EmployeeID = E.EmployeeID
             INNER JOIN Course Co ON C.CourseID = Co.CourseID
             INNER JOIN Period P ON C.PeriodID = P.PeriodID
             WHERE C.Status = 1";
...
echo "<tr>";
echo "<th><p>ClassID</p></th>";
echo "<th><p>Employee Name</p></th>";
echo "<th><p>Course Name</p></th>";
echo "<th><p>Period Name</p></th>";

while ($registro = $datos->fetch_assoc()) {
    echo '<tr>';
    echo "<td>" . $registro["ClassID"] . "</td>";
    echo "<td>" . $registro["EmployeeName"] . "</td>";
    echo "<td>" . $registro["CourseName"] . "</td>";
    echo "<td>" . $registro["PeriodName"] . "</td>";
    echo '<td><a href="FormUpdClass.php?ClassID=' . $registro["ClassID"] . '"><i class="bi bi-pencil-square"></i></td>';
    echo '<td><a href="DelClass.php?ClassID=' . $registro["ClassID"] . '" onclick="return confirm(\'¿Estás seguro de que deseas eliminar este registro?\')"><i class="bi bi-trash-fill"></i></td>';
    echo "</tr>";
}
echo "</table>";

echo '<td><a href="FormInserClass.php"><i class="bi bi-plus-square"></i></td>';
