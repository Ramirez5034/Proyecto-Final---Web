...
//------------------------------------------------------------------------------------------------------
$consulta = "SELECT CareerService.CareerServiceID, Career.Name AS CareerName, Employee.FirstName, CareerService.ServiceDate, CareerService.Description, CareerService.State
FROM CareerService
INNER JOIN Career ON CareerService.CareerID = Career.CareerID
INNER JOIN Employee ON CareerService.EmployeeID = Employee.EmployeeID
WHERE CareerService.status = 1";

$datos = $conn->query($consulta);
echo '<table class="table">';
echo "<tr>";
echo "<th><p>Career</th></p>";
echo "<th><p>Employee</th></p>";
echo "<th><p>Service Date</th></p>";
echo "<th><p>Description</th></p>";
echo "<th><p>State</th></p>";
while ($registro = $datos->fetch_assoc()) {
    echo '
        <tr>           
            <td>' . $registro["CareerName"] . '</td>
            <td>' . $registro["FirstName"] . '</td>
            <td>' . $registro["ServiceDate"] . '</td>
            <td>' . $registro["Description"] . '</td>
            <td>' . $registro["State"] . '</td>
            <td><a href="FormUpdCareerService.php?CareerServiceID=' . $registro["CareerServiceID"] . '"><i class="bi bi-pencil-square"></i></a></td>
            <td><a href="DelCareerService.php?CareerServiceID=' . $registro["CareerServiceID"] . '" onclick="return confirm(\'¿Estás seguro de que deseas eliminar este registro?\')">
            <i class="bi bi-trash-fill"></i></a></td>';
    echo "</tr>";
}
echo "</table>";

echo '<td><a href="ForminserCareerService.php"><i class="bi bi-plus-square"></i>';
$conn->close();
